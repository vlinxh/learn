angular.module('tm.pagination', []).directive('tmPagination', [function () {
    return {
        ...
        ' 共<strong>{{ conf.totalItems }}</strong>条/<strong>{{ conf.numberOfPages }}</strong>页 ' +
        ...
        link: function (scope, element, attrs) {
                ...
                if (conf.numberOfPages <= conf.pagesLength) {
                    ...
                    if (conf.currentPage <= offset) {
                        ...
                        for (i = 1; i <= offset + 1; i++) {
                            scope.pageList.push(formatPage(i));
                        }
                        // scope.pageList.push('...');
                        ...
                        for (i = offset + 1; i >= 1; i--) {
                            scope.pageList.push(formatPage(conf.numberOfPages - i));
                        }
                        scope.pageList.push(formatPage(conf.numberOfPages));
                    } else {
                        // 最后一种情况，两边都有...
                        // scope.pageList.push(1);
                        // scope.pageList.push('...');
                        ...
                        for (i = Math.ceil(offset / 2); i >= 1; i--) {
                            scope.pageList.push(formatPage(conf.currentPage - i));
                        }
            ...
            function formatPage(page) {
                var pagesLength = conf.numberOfPages.toString().length;
                var currentLength = page.toString().length;
                for(var i = 0; i < (pagesLength - currentLength) ; i++) {
                    page = "0" + page;
                }
                return page;
            }
